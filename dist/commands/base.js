...
module.exports = function (command) {
    return {
        ...
        run: async function (parsedCommand, message, client, settings) {
            try {
                ...
                await this.channel.send({
                    embeds: [failureEmbed({
                            description: e instanceof CommandError
                                ? e.message
        ...
        validateGuildCooldowns: function () {
            ...
            if (commandCooldowns.has(this.guild.id)) {
                ...
                if (now < expiration) {
                    ...
                    throw new CommandMemberPermissionsError(`**Command on server cooldown.** Please wait ${remaining} ` +
        ...
        validateMemberPermissions: async function () {
            if (isEmpty(this.memberPermissions))
                return true;
            if (!this.memberPermissions.some((p) => this.client.utils.memberHasPermission(this.member, p))) {
                throw new CommandMemberPermissionsError("**Insufficient permissions.** " +
        ...
        validateChannelPermissions: function () {
            if (this.guildOnly && !this.client.utils.isGuildTextChannel(this.channel)) {
                throw new CommandChannelPermissionsError("This command is only allowed in server text channels.");
            }
        },
        get author() {
