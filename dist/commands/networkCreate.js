...
module.exports = base({
    ...
    exec: async function () {
        const channel = this.client.repo.GetGuildChannel(this.guild, this.parsedCommand.reader.getChannelID());
        const settings = this.client.repo.GetGuildClientSettings(this.guild.id);
        try {
            const { passphrase, uuid } = await CreateNetwork(this.guild, this.author, {
                ...settings,
            ...
            this.channel.send({
                embeds: [successEmbed({
                        ...
                        command: true,
                        description: "Keep these information in a safe place and then delete this message.\n\n" +
            ...
            throw new CommandError(e.code === pgErrorCodes.UNIQUE_VIOLATION
