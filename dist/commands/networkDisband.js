...
const { DisbandNetwork, GetGuildOwnedNetwork, GetNetworkGuilds, } = require("../lib/resourceRepo/index");
...
module.exports = base({
    ...
    exec: async function () {
        ...
        const channels = await Promise.all(guilds.map(async (guild) => {
            const settings = await this.client.repo.GetGuildClientSettings(guild.id);
            try {
                return this.client.repo.GetGuildChannel(guild, settings.channelId, true);
            }
            catch (e) {
    ...
    handleAcceptance: function ({ announcementsChannel, network, targetChannels, unreachableGuilds, }) {
        return async () => {
            await DisbandNetwork(network);
            await announcementsChannel.send(`Your network is now gone. ${unreachableGuilds.length
                ? `Unreachable servers: ${unreachableGuilds.join(", ")}`
                : ""}`);
            for (const channel of targetChannels) {
                await channel.send(`Your **Komvos** network \`${network.uuid}\` ` +
...
//# sourceMappingURL=networkDisband.js.map