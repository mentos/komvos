...
const { GetGuildActiveNetwork, GetNetworkGuilds, } = require("../lib/resourceRepo/index");
    ...
    exec: async function () {
        ...
        this.channel.send({
            embeds: [successEmbed({
                    ...
                    titlePrefix: "",
                    description: `${description} ${unreachableGuilds.length
...
const truncate = (text, length = 25) => `${text.slice(0, length)}${text.length > length ? "…" : ""}`.padEnd(length + 5, " ");
