...
const { AddGuildToNetwork, GetGuildNetworksCount, GetGuildOwnedNetwork, } = require("../lib/resourceRepo/index");
...
module.exports = base({
    ...
    exec: async function () {
        ...
        const { channelId } = await this.client.repo.GetGuildClientSettings(this.guild.id);
        this.client.repo.GetGuildChannel(this.guild, channelId);
        if ((await GetGuildNetworksCount(targetGuild.id)).count > 0) {
            throw new CommandTargetError(`Server **${targetGuild.name}** is already a member of a **Komvos** network. ` +
                "Please let the server administrator know and try again when everything is setup.");
        }
        const { allowInvites, channelId: targetChannelId, } = await this.client.repo.GetGuildClientSettings(targetGuild.id);
        validateGuildInvitationStatus(targetGuild)(allowInvites);
        const targetChannel = this.client.repo.GetGuildChannel(targetGuild, targetChannelId);
        await new GuildBroadcastPrompt({
            ...
            onAccept: this.handleAcceptance.bind(this, targetGuild, targetChannel, network, this.channel),
            onReject: this.handleRejection.bind(this, targetGuild, this.channel),
            promptContent: {
                content: `User **${this.author.tag}** of the **${this.guild.name}** server ` +
    ...
    handleAcceptance: async function (targetGuild, targetChannel, network, announcementsChannel) {
        await AddGuildToNetwork(targetGuild.id, network.id);
        await announcementsChannel.send(`Folks at server **${targetGuild.name}** accepted your **Komvos** network invitation!`);
        await targetChannel.send(`Congrats! You are now member of **Komvos** network: \`${network.uuid}\`!`);
    },
    handleRejection: async function (targetGuild, announcementsChannel) {
        await announcementsChannel.send(`Folks at server **${targetGuild.name}** rejected your **Komvos** network invitation.`);
    },
});
const validateGuildInvitationStatus = (guild) => (allowInvites) => {
    if (!allowInvites) {
        throw new CommandError(`Server **${guild.name}** has turned off network invitations for **Komvos**. ` +
            "Please let the server administrator know and try again when everything is setup.");
    }
};
//# sourceMappingURL=networkInvite.js.map