...
const { GetGuildOwnedNetwork, GetNetworkGuildIdForKick, RemoveGuildFromNetwork, } = require("../lib/resourceRepo/index");
...
module.exports = base({
    ...
    exec: async function () {
        ...
        await new GuildBroadcastPrompt({
            ...
            onReject,
            promptContent: {
                content: `You're about to kick server **${targetGuild.name}** from your network \`${network.uuid}\`. **Are you sure?**` +
                    `${!targetChannel
    ...
    handleAcceptance: function ({ announcementsChannel, network, targetChannel, targetGuild, }) {
        return async () => {
            await RemoveGuildFromNetwork(targetGuild.id, network.id);
            await announcementsChannel.send(`Server **${targetGuild.name}** is no longer part of your network.` +
                (!targetChannel
                    ? ` Server **${targetGuild.name}** was **NOT** notified as there is NO ` +
                        `announcements channel setup on that server.`
                    : ""));
            if (targetChannel)
                await targetChannel.send(`You have been **kicked** from **Komvos** network: \`${network.uuid}\`. ` +
...
//# sourceMappingURL=networkKick.js.map