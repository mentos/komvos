...
const { GetGuildActiveNetwork, RemoveGuildFromNetwork, } = require("../lib/resourceRepo/index");
...
module.exports = base({
    ...
    exec: async function () {
        ...
        if (network.owning_guild_id === this.guild.id) {
            throw new base.CommandError("A network admin cannot leave a network, only disband it.");
        }
        const targetGuild = await this.client.repo.GetGuild(network.owning_guild_id);
        let targetChannel;
        try {
            const settings = await this.client.repo.GetGuildClientSettings(targetGuild.id);
            targetChannel = this.client.repo.GetGuildChannel(targetGuild, settings.channelId, true);
        }
        catch (e) {
            console.trace(e);
        }
        ...
        await new GuildBroadcastPrompt({
            ...
            onReject,
            promptContent: {
                content: `You're about to leave network \`${network.uuid}\` owned by server **${targetGuild.name}**. **Are you sure?**` +
                    `${!targetChannel
    ...
    handleAcceptance: function ({ announcementsChannel, network, guild, targetChannel, targetGuild, }) {
        return async () => {
            await RemoveGuildFromNetwork(guild.id, network.id);
            await announcementsChannel.send(`You left network **\`${network.uuid}\`**. You will no longer receive broadcasts from it. ` +
                ...
                (!targetChannel
                    ...
                    : ""));
            if (targetChannel)
                await targetChannel.send(`Server **${guild.name}** has left your network. ` +
...
//# sourceMappingURL=networkLeave.js.map