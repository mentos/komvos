...
const { AddBanBroadcast, GetBanBroadcast, GetGuildActiveNetwork, GetNetworkGuilds, } = require("./resourceRepo/index");
...
module.exports = async ({ banReason, bannedUser, client, fromCommand = false, guild, guildChannel, }) => {
    ...
    if (!isEmpty(banBroadcast)) {
        const description = `🔔 _Broadcast for **${banBroadcast.banned_tag}** ` +
    ...
    let reportType = "React to least one of the following reasons and " +
    ...
    const reactionListener = new ReactionHandler(report, (userId) => userId !== report.author.id, false, { time: 300000 });
    const reactions = [...Object.keys(REPORT_OPTIONS), "✅"];
    for (let i = 0; i < reactions.length; i++) {
        await report.addReaction(reactions[i]);
    }
    reactionListener.on("reacted", async (event) => {
        const reactionsLength = reactionListener.collected.filter(({ emoji }) => emoji.name !== "✅" && Object.keys(REPORT_OPTIONS).includes(emoji.name)).length;
    ...
    reactionListener.on("end", async (collected, reason) => {
        ...
        try {
            ...
            if (e.code === pgErrorCodes.UNIQUE_VIOLATION) {
                const description = `🔔 _A broadcast for ${banInfo.banned_tag} ` +
        ...
        await channel.send({
            embeds: [new EmbedBuilder({
                    description: `✅ _**Ban for ${banInfo.banned_tag} broadcasted to network**_` +
...
//# sourceMappingURL=BroadcastBanReport.js.map