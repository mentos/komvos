...
const { GetBanBroadcast, GetGuildActiveNetwork, GetNetworkGuilds, RevokeBanBroadcast, } = require("../lib/resourceRepo/index");
async function BroadcastBanRevokeReport({ bannedUserId, channel, client, guild, reason, }) {
    ...
    if (isEmpty(banBroadcast)) {
        const description = "🔔 _Revoke broadcasts can be performed by original broadcasters or network administrator._";
        const embed = new EmbedBuilder({ color: 16763904, description }).sendable;
        await channel.send({ embeds: [embed] });
        return;
    }
    if (banBroadcast.guild_id !== guild.id &&
        banBroadcast.guild_id !== network.owning_guild_id) {
        const description = `🔔 _Cannot find a ban broadcast for user ID **${bannedUserId}**._ ` +
    ...
    await channel.send({
        embeds: [new EmbedBuilder({
                description: `✅ _**Ban revoke for ${banBroadcast.banned_tag} was broadcasted to network**_` +
...
//# sourceMappingURL=BroadcastBanRevokeReport.js.map