...
class GuildBroadcastPrompt {
    constructor({ broadcastChannel, expirationMessage, onAccept, onReject, promptContent, replyToBroadcastChannel = null, targetChannel, targetExpirationMessage, time = 6000, }) {
    ...
    async send() {
        this.promptMessage = await this.targetChannel
            .send(typeof this.promptContent === "string"
            ? this.promptContent
            : { ...this.promptContent })
            .then(async (m) => {
        ...
        const reactionListener = new ReactionHandler(this.promptMessage, (userId) => userId !== this.promptMessage.author.id, false, { maxMatches: 1, time: this.time });
...
//# sourceMappingURL=GuildBroadcastPrompt.js.map